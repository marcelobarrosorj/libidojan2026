...
export const createPayment = async (
  ...
  try {
    ...
    const { customerTaxId, planId } = req.body;
    ...
    if (!planId || typeof planId !== 'string') {
      return res.status(400).json({
        error: 'Escolha um plano antes de gerar o PIX.'
      });
    }
    ...
    /*
      Preco e duracao vêm SEMPRE da tabela subscription_plans.
      O frontend envia apenas o plan_id escolhido — nunca o valor.
    */
    ...
    if (error && error.response && error.response.status) {
      ...
      const lista = Array.isArray(dados.error_messages) ? dados.error_messages : [];
      const safeErrors = lista.map((e: any) => ({
        code: e && e.code,
        error: e && e.error,
        description: e && e.description,
        parameter_name: e && e.parameter_name
      }));
      console.error('PAGBANK_REQUEST_ERROR', { status: error.response.status, errors: safeErrors });
    }
...
export const pagbankWebhook = async (
  ...
  try {
    ...
    */
    ...
    /*
      O valor pago deve bater com o valor registrado na transacao
      criada no checkout (que ja carrega o plano escolhido).
      Funciona para o day use (19,90) e para o mensal (49,90).
    */
    ...
    if (!tx) {
      console.error('PAGBANK FRAUD_ALERT: transacao nao encontrada ' + orderId);
      return res.status(200).send('OK');
    }

    const storedCents = Math.round(Number(tx.amount) * 100);
    if (Math.round(amountPaid * 100) !== storedCents) {
      console.error(
...
export const getPaymentStatus = async (
  ...
  try {
    ...
    const supabase = getAdminSupabase();
    /*
      Esta rota NÃO consulta o PagBank e NÃO ativa Premium.
...
export const getPlans = async (
  ...
  try {
    ...
    console.error('Erro em getPlans:', error.message);
    return res.status(500).json({ error: 'Erro interno.' });
  }
};
