...
import { Rss, Radio, MessageCircle, Menu, X, Users, Trophy, Calendar, UserPlus, Settings, User, Shield } from 'lucide-react';
    ...
    <>
      {showMenu && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md max-h-[85dvh] bg-[var(--libido-bg)] border border-[var(--libido-border)] rounded-3xl flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-[var(--libido-border)]">
              <h2 className="text-sm font-black text-[var(--libido-accent)] uppercase tracking-widest italic">
                Menu da Matriz
              </h2>

              <button
                onClick={() => setShowMenu(false)}
                className="text-[var(--libido-muted)] opacity-60 hover:text-[var(--libido-text)]"
            ...
            <nav className="flex-1 p-3 space-y-1 overflow-y-auto no-scrollbar">
              {menuItems.map((item) => (
                ...
                >
                  ...
                  {item.premium && (
                    <span className="ml-auto text-[7px] font-black bg-[var(--libido-accent)]/20 text-[var(--libido-accent)] px-1.5 py-0.5 rounded">
                      VIP
                    </span>
                  )}
                </button>
        ...
        {mainTabs.map((tab) => (
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all ${
              activeTab === tab.id
                ? 'text-[var(--libido-accent)]'
                : 'text-[var(--libido-muted)]/50 hover:text-[var(--libido-muted)]/80'
            }`}
          >
