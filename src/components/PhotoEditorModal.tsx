...
import { RotateCw, Grid, Square, ShieldAlert, Eye, EyeOff, Save, X, Undo, Sliders } from 'lucide-react';

interface PhotoEditorModalProps {
  file: File;
  onClose: () => void;
  onSave: (editedBlob: Blob, isPrivate: boolean) => Promise<void>;
}

export function PhotoEditorModal({ file, onClose, onSave }: PhotoEditorModalProps) {
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (tool === 'pixelate') {
      ...
      if (tempCtx) {
        ...
        const pixelScale = 8; 
        const sw = Math.max(1, w / pixelScale);
        const sh = Math.max(1, h / pixelScale);
        ctx.drawImage(tempCanvas, 0, 0, w, h, startX, startY, sw, sh);
        ctx.drawImage(canvas, startX, startY, sw, sh, startX, startY, w, h);
      }
  ...
  return (
    <div className="fixed inset-0 bg-[var(--libido-surface)]/95 z-[150] flex flex-col justify-between p-4 backdrop-blur-md animate-in fade-in">
      <div className="flex items-center justify-between py-2 border-b border-[var(--libido-border)]">
        ...
        <button onClick={onClose} className="text-[var(--libido-muted)] opacity-60 hover:text-[var(--libido-text)] p-2">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="bg-[var(--libido-surface-2)] border border-[var(--libido-border)] rounded-3xl p-4 space-y-4 max-w-sm mx-auto w-full">
        ...
        <div className="flex items-center justify-between border-t border-[var(--libido-border)] pt-3">
          <div>
            <p className="text-[9px] font-black uppercase tracking-wider text-[var(--libido-text)]">Privacidade</p>
            <p className="text-[8px] text-[var(--libido-muted)] opacity-60 font-bold">Secretas exigem VIP</p>
          </div>
          <div className="flex gap-2">
            <button
              type="button"
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${
                !isPrivate ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' : 'bg-transparent text-[var(--libido-muted)] opacity-50 border-[var(--libido-border)]'
              }`}
            >
              <Eye size={12} />
              Pública
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${
                isPrivate ? 'bg-[var(--libido-accent)]/10 text-[var(--libido-accent)] border-[var(--libido-accent)]/20' : 'bg-transparent text-[var(--libido-muted)] opacity-50 border-[var(--libido-border)]'
              }`}
            >
              <EyeOff size={12} />
              Secreta
